...
def start(message):
    ...
    if bot_state.CORRECT_ID > -1:
        bot.edit_message_reply_markup(chat_id=message.chat.id, message_id=bot_state.CORRECT_ID, reply_markup=None)
        bot_state.CORRECT_ID = -1
    if bot_state.SKIP_ID > -1:
        bot.edit_message_reply_markup(chat_id=message.chat.id, message_id=bot_state.SKIP_ID, reply_markup=None)
        bot_state.SKIP_ID = -1
...
def input_date(message, name): 
    if message.text[1:] in my_commands: handle_command(message)
        ...
        if check_date(date):
            ...
            bot_state.SKIP_ID = message.message_id + 1
...
def check_info(message, name, date, add):
    ...
    bot_state.CORRECT_ID = message.message_id + 1
